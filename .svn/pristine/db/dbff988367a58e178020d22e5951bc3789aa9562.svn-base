 ...
 * 
 */
public class SessionManager extends DefaultListenable<NegotiationEvent> implements Runnable {
	 ...
	 */
	public SessionManager(List<NegotiationPartyInternal> theparties, Session session, ExecutorWithTimeout exec) {
		...
		for (NegotiationPartyInternal p : theparties) {
			...
			profiles.put(p.getID().toString(), "Profile" + p.getUtilitySpace().getName().hashCode());
		}
	}
	...
	/**
	 * Runs the negotiation session and wait for it to complete. If an error
	 * occurs, we still try to log the last agreement and call the parties that
	 * the session ended.
	 * 
	...
	public void runAndWait()
			throws ActionException, InterruptedException, ExecutionException, NegotiationPartyTimeoutException {
		...
		try {
			...
			Bid agreement = session.getInfo().getProtocol().getCurrentAgreement(session, parties);
			notifyChange(new MultipartySessionEndedEvent(session, agreement, partiesInternal));
	...
	private void savePartiesInfo(Bid agreementBid) {
		for (final NegotiationPartyInternal party : partiesInternal) {
			final Tuple<Bid, Double> agreement = new Tuple<>(agreementBid, party.getUtility(agreementBid));
			try {
				new ExecutorWithTimeout(SAVE_INFO_MAXTIME).execute("saving info for " + party.getID(),
						new Callable<String>() {
				...
				notifyChange(new RecoverableSessionErrorEvent(session, party, e));
			}
	...
	private void callPartiesSessionEnded(final Bid agreement) {
		for (final NegotiationPartyInternal party : partiesInternal) {
			try {
				Map<String, String> result = new ExecutorWithTimeout(SESSION_ENDED_MAXTIME)
						.execute("session ended " + party.getID(), new Callable<Map<String, String>>() {

							@Override
							public Map<String, String> call() throws Exception {
								return party.getParty().negotiationEnded(agreement);
							}
						});

				if (result != null && !result.isEmpty()) {
					notifyChange(new AgentLogEvent(party.getID().toString(), result));
				}
			} catch (ExecutionException | TimeoutException e1) {
				notifyChange(new RecoverableSessionErrorEvent(session, party, e1));
			}
		}
	}
	...
	private void executeProtocol()
		...
		do {
			...
			// generate new round
			Round round = session.getInfo().getProtocol().getRoundStructure(parties, session);
			...
			for (Turn turn : round.getTurns()) {
				...
				if (session.getInfo().getProtocol().isFinished(session, parties)) {
					break;
				}
			}
			if (checkDeadlineReached())
		...
		} while (!session.getInfo().getProtocol().isFinished(session, parties) && !checkDeadlineReached());
	 ...
	 * @throws TimeoutException
	 */
	private void handleBeforeSession()
		...
		Map<NegotiationParty, List<Action>> preparatoryActions = session.getInfo().getProtocol().beforeSession(session,
				negoparties);

		for (final NegotiationParty party : preparatoryActions.keySet()) {
			for (final Action act : preparatoryActions.get(party)) {
				try {
					executor.execute(getPartyID(party).toString(), new Callable<Object>() {
						@Override
					...
					throw new NegotiationPartyTimeoutException(party, "party timed out in the before-session update",
							e);
				}
			}
	...
	private void doPartyTurn(int turnNumber, Turn turn)
		...
		notifyChange(new MultipartyNegoActionEvent(action, session.getRoundNumber(), session.getTurnNumber(),
				session.getTimeline().getTime(), partiesInternal,
				session.getInfo().getProtocol().getCurrentAgreement(session, parties)));
	}
	...
	private boolean checkDeadlineReached() {
		...
		if (session.isDeadlineReached()) {
			...
			if (session.getDeadlines().getType() == DeadlineType.TIME) {
				double runTimeInSeconds = (Integer) session.getDeadlines().getValue();
	 ...
	 * @throws TimeoutException
	 */
	private Action requestAction(final NegotiationParty party, final List<Class<? extends Action>> validActions)
			throws ActionException, InterruptedException, ExecutionException, NegotiationPartyTimeoutException {

		Action action;
		try {
			action = executor.execute(getPartyID(party).toString(), new Callable<Action>() {
				@Override
			...
			String msg = "Negotiating party " + getPartyID(party) + " timed out in chooseAction() method.";
	 ...
	 * 
	 */
	private Action checkAction(NegotiationParty party, Action action, List<Class<? extends Action>> validActions)
			throws ActionException {
		if (action == null || action.getAgent() == null || !validActions.contains(action.getClass())) {
		...
		if (!action.getAgent().equals(getPartyID(party))) {
			throw new InvalidActionContentsError(getPartyID(party),
					"partyID " + getPartyID(party) + " does not match agentID in action" + action.getAgent());
		}
		return action;
	}
	...
	private void updateListeners(final NegotiationParty actionOwner, final Action action)
			throws NegotiationPartyTimeoutException, ExecutionException, InterruptedException {
		Map<NegotiationParty, List<NegotiationParty>> listeners = session.getInfo().getProtocol()
		...
		if (listeners.get(actionOwner) != null)
			for (final NegotiationParty observer : listeners.get(actionOwner)) {
				try {
					executor.execute(getPartyID(actionOwner).toString(), new Callable<Object>() {
						@Override
						public Object call() {
							observer.receiveMessage(getPartyID(actionOwner), action);
							return null;
						}
					});
				} catch (TimeoutException e) {
					String msg = String.format("Negotiating party %s timed out in receiveMessage() method.",
							getPartyID(observer));
	 ...
	 */
	private NegotiationPartyInternal getNegoPartyInternal(NegotiationParty needed) {
		for (NegotiationPartyInternal party : partiesInternal) {
		...
		throw new IllegalStateException(
				"The referenced NegotiationParty " + needed + " is not one of the actual running parties.");
	}
