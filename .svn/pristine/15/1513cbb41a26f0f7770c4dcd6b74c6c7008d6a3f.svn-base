...
import negotiator.boaframework.BoaParty;
...
public class BoaPartyRepItem extends ParticipantRepItem {
	...
	public BoaPartyRepItem(String partyName) throws InstantiateException {
		...
		acceptanceStrategy = new BoaWithSettingsRepItem<>(BoaType.ACCEPTANCESTRATEGY);
		opponentModel = new BoaWithSettingsRepItem<>(BoaType.OPPONENTMODEL);
		omStrategy = new BoaWithSettingsRepItem<>(BoaType.OMSTRATEGY);
	}

	public BoaPartyRepItem(String partyName, BoaWithSettingsRepItem<OfferingStrategy> boa1,
			BoaWithSettingsRepItem<AcceptanceStrategy> boa2, BoaWithSettingsRepItem<OpponentModel> boa3,
			BoaWithSettingsRepItem<OMStrategy> boa4) {
		if (boa1 == null || boa2 == null || boa3 == null || boa4 == null) {
			throw new NullPointerException("one of boa arguments = null");
		}
		this.partyName = partyName;
	...
	public BoaWithSettingsRepItem<? extends BOA> getStrategy(BoaType strategyType) {
		...
		throw new IllegalArgumentException("There is no strategy for " + strategyType);
	}
	...
	public NegotiationParty load() throws InstantiateException {
		...
		Map<String, Double> asparams = acceptanceStrategy.getParameters().asMap();
		...
		return new BoaParty(as, asparams, os, osparams, om, omparams, oms, omsparams);
	}

	@Override
	public String getUniqueName() {
		return "boa-" + biddingStrategy.getUniqueName() + "-" + acceptanceStrategy.getUniqueName() + "-"
				+ opponentModel.getUniqueName() + "-" + omStrategy.getUniqueName();
	}

	@Override
	public String toString() {
		return "BoaPartyRepItem[" + partyName + "," + biddingStrategy + "," + acceptanceStrategy + "," + opponentModel
				+ "," + omStrategy + "]";
	}
