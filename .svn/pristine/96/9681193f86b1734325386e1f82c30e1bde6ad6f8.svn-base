...
public class Repository<T extends RepItem> extends DefaultListenable<BoaPartyRepItem> {
	 ...
	 */
	public static <T1 extends RepItem> Repository<T1> fromFile(String fileName) {
		try {
			JAXBContext jaxbContext = JAXBContext.newInstance(Repository.class, PartyRepItem.class,
					ProfileRepItem.class, MultiPartyProtocolRepItem.class, DomainRepItem.class, AgentRepItem.class);
			Unmarshaller unmarshaller = jaxbContext.createUnmarshaller();
			unmarshaller.setEventHandler(new javax.xml.bind.helpers.DefaultValidationEventHandler());
			Repository<T1> rep = (Repository<T1>) (unmarshaller.unmarshal(new File(fileName)));
			rep.fileName = fileName;
			return rep;
		} catch (JAXBException e) {
			throw new RepositoryException("Failed to unmarshall " + fileName, e);
		}
	}
	...
	public void save() {
		try {
			JAXBContext jaxbContext = JAXBContext.newInstance(Repository.class, ProfileRepItem.class,
					DomainRepItem.class, AgentRepItem.class, PartyRepItem.class, ProtocolRepItem.class,
					MultiPartyProtocolRepItem.class);
		...
		try {
			if ((sourceFolder != null) && (!sourceFolder.equals(""))) {
				...
				if (UTILITYSPACETYPE.getUtilitySpaceType(file) == UTILITYSPACETYPE.NONLINEAR) // RA
					...
					us = new ConstraintUtilitySpace(domain, sourceFolder + "/" + file); // RA
				else
