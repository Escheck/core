...
public class DomainImpl implements Domain, Serializable {
	...
	public final Objective getObjectivesRoot() {
		return fObjectivesRoot; // TODO hdevos this could be done in a more
								// elegant way. To discuss with Richard.
	}
	...
	private final void loadTreeFromXML(SimpleElement pRoot) {
		...
		// causes error. the []s seem to cause a classcastexception.
		// SimpleElement[] root =
		// (SimpleElement[])(pRoot.getChildByTagName("objective")); //Get the
		// actual root Objective.
		SimpleElement root = (SimpleElement) (pRoot.getChildByTagName("objective")[0]); // Get
																						// the
																						// actual
																						// root
																						// Objective.
	...
	private final Objective buildTreeRecursive(SimpleElement currentLevelRoot, Objective currentParent) {
		 ...
		 * Integer sint = new Integer(s); int sintint = sint; //Check of this
		 * cast works!
		 */
		...
		for (int i = 0; i < currentLevelObjectives.length; i++) {
			...
			/*
			 * Double weight = new
			 * Double(childObjectives.getAttribute("weight")); //TODO check if
			 * weigth is the same things as value!
		...
		for (int j = 0; j < currentLevelIssues.length; j++) {
			...
			if (type == null) { // No value type specified.
				// new Warning("Type not specified in template file of " + name
				// +
				// " at " + index);
		...
		for (Issue lIssue : getIssues()) {
			...
			case INTEGER:
				...
				if (lOptionIndex >= lNrOfOptions)
					lOptionIndex = lNrOfOptions - 1;
				// values[i]= new
				// ValueInteger(((IssueInteger)lIssue).getLowerBound()+lOptionIndex);
			...
			case REAL:
				...
				if (lOptionIndex >= lNrOfOptions)
					lOptionIndex = lNrOfOptions - 1;
				// values[i]= new
				// ValueReal(lIssueReal.getLowerBound()+lOneStep*lOptionIndex);
